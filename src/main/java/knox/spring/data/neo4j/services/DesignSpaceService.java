...
public class DesignSpaceService {
    ...
    public DesignSpace convertDeviceToDesignSpace(Device device,
                                                  Set<Part> parts) {
    ...
    public void importCSV(List<InputStream> inputCSVStreams,
                          String outputSpacePrefix, boolean isMerge) {
        ...
        for (InputStream inputCSVStream : inputCSVStreams) {
            try {
                ...
                BufferedReader csvReader =
                    new BufferedReader(new InputStreamReader(inputCSVStream));

                if ((csvLine = csvReader.readLine()) != null) {
                    ArrayList<String> csvArray = csvToArrayList(csvLine);

                    if (csvArray.size() > 0) {
                        if (csvArray.get(0).equals("id") &&
                            csvArray.get(1).equals("role") &&
        ...
        for (BufferedReader designReader : designReaders) {
            try {
                csvSpaces.addAll(processCSVDesigns(
                    designReader, outputSpacePrefix, compIDToRole));
            } catch (IOException e) {
                e.printStackTrace();
            } finally {
                try {
    ...
    public List<DesignSpace> processCSVDesigns(
        BufferedReader csvReader, String outputSpacePrefix,
        ...
        while ((csvLine = csvReader.readLine()) != null) {
            ...
            if (csvArray.size() > 0 && csvArray.get(0).length() > 0) {
                ...
                DesignSpace outputSpace =
                    new DesignSpace(outputSpacePrefix + j);
                ...
                for (int i = 0; i < csvArray.size(); i++) {
                    if (csvArray.get(i).length() > 0) {
                        ...
                        outputPredecessor.createEdge(outputNode, compIDs,
                                                     compRoles);
    ...
    public HashMap<String, String> processCSVComponents(
        BufferedReader csvReader) throws IOException {
        HashMap<String, String> compIDToRole = new HashMap<String, String>();
    ...
    public void mergeSBOL(List<InputStream> inputSBOLStreams,
                          String outputSpaceID, String authority)
        ...
        for (InputStream inputSBOLStream : inputSBOLStreams) {
            ...
            Set<ComponentDefinition> compDefs =
                getDNAComponentDefinitions(sbolDoc);

            for (ComponentDefinition compDef : compDefs) {
                if (!hasDesignSpace(compDef.getIdentity().toString())) {
                    DesignSpace outputSpace =
                        new DesignSpace(compDef.getIdentity().toString());

                    List<ComponentDefinition> leafDefs =
                        flattenComponentDefinition(compDef);
                    ...
                    for (int i = 0; i < leafDefs.size(); i++) {
                        ...
                        ArrayList<String> compRoles =
                            new ArrayList<String>(convertSOIdentifiersToNames(
                                leafDefs.get(i).getRoles()));
                    ...
                    outputSpace.createHeadBranch(
                        compDef.getIdentity().toString());

                    saveDesignSpace(outputSpace);

                    commitToBranch(outputSpace.getSpaceID(),
                                   outputSpace.getHeadBranch().getBranchID());
                }
            }
    ...
    private List<ComponentDefinition> flattenComponentDefinition(
        ComponentDefinition rootDef) {
        List<ComponentDefinition> leafDefs =
        ...
        while (defStack.size() > 0) {
            ...
            if (seqAnnos.size() == 0) {
                ...
                HashMap<String, Integer> compIDToStart =
                ...
                for (SequenceAnnotation seqAnno : seqAnnos) {
                    ...
                    while (i < sortedSubComps.size() &&
                           compIDToStart.get(subComp.getIdentity().toString()) <
                               compIDToStart.get(sortedSubComps.get(i)
                                                     .getIdentity()
                                                     .toString())) {
                        i++;
                    }
    ...
    private Set<ComponentDefinition> getDNAComponentDefinitions(
        SBOLDocument sbolDoc) {
        Set<ComponentDefinition> dnaCompDefs =
            new HashSet<ComponentDefinition>();
    ...
    public void insertBranch(String targetSpaceID, String inputBranchID1,
        ...
        Node nodeCopy = findNodeCopy(targetSpaceID, inputBranchID2,
                                     targetNodeID, outputBranchID);

        deleteNodeCopyIndices(targetSpaceID, outputBranchID);

        if (nodeCopy != null) {
            Set<Edge> removedEdges = removeOutgoingEdges(
                targetSpaceID, outputBranchID, nodeCopy.getNodeID());

            if (removedEdges.size() > 0) {
                for (Node acceptNode1 : acceptNodes1) {
                    deleteNodeType(targetSpaceID, outputBranchID,
                                   acceptNode1.getNodeID());
                }
            } else {
                Set<Node> acceptNodes =
                    getAcceptNodes(targetSpaceID, outputBranchID);
            ...
            if (startNode1 != null) {
                deleteNodeType(targetSpaceID, outputBranchID,
                               startNode1.getNodeID());
                createEdge(targetSpaceID, outputBranchID, nodeCopy.getNodeID(),
                           startNode1.getNodeID());
            }
            for (Node acceptNode1 : acceptNodes1) {
                for (Edge removedEdge : removedEdges) {
                    if (removedEdge.hasComponentIDs() &&
                        removedEdge.hasComponentRoles()) {
                        createComponentEdge(targetSpaceID, outputBranchID,
                                            acceptNode1.getNodeID(),
                                            removedEdge.getHead().getNodeID(),
                                            removedEdge.getComponentIDs(),
                                            removedEdge.getComponentRoles());
                    } else {
    ...
    public void joinBranches(String targetSpaceID,
                             List<String> inputBranchIDs) {
    ...
    public void joinBranches(String targetSpaceID, List<String> inputBranchIDs,
        ...
        for (int i = outputIndex + 1; i < inputBranchIDs.size(); i++) {
            ...
            Set<String> startNodeIDs1 =
                getStartNodeIDs(targetSpaceID, outputBranchID);

            Set<String> acceptNodeIDs1 =
                getAcceptNodeIDs(targetSpaceID, outputBranchID);
        ...
        for (int i = 0; i < outputIndex; i++) {
            ...
            Set<String> startNodeIDs2 =
                getStartNodeIDs(targetSpaceID, outputBranchID);

            Set<String> acceptNodeIDs2 =
                getAcceptNodeIDs(targetSpaceID, outputBranchID);
    ...
    public void mergeBranches(String targetSpaceID, List<String> inputBranchIDs,
                              boolean isIntersection, boolean isCompleteMatch,
                              int strength, int degree) {
        mergeBranches(targetSpaceID, inputBranchIDs, RESERVED_ID,
                      isIntersection, isCompleteMatch, strength, degree);
    }

    public void mergeBranches(String targetSpaceID, List<String> inputBranchIDs,
                              String outputBranchID, boolean isIntersection,
                              boolean isCompleteMatch, int strength,
                              int degree) {
        DesignSpace targetSpace = loadDesignSpace(targetSpaceID, 5);
        ...
        Branch outputBranch =
            targetSpace.createBranch(outputBranchID, maxIDIndex);
                ...
                if (degree >= 1) {
                    if (degree == 2) {
                        if (inputCommit.getSnapshot().hasNodes()) {
                            inputStarts.addAll(
                                inputCommit.getSnapshot().getNodes());
                        }
                    } else {
                        inputStarts.addAll(
                            inputCommit.getSnapshot().getStartNodes());
                    }

                    if (outputCommit.getSnapshot().hasNodes()) {
                        outputStarts.addAll(
                            outputCommit.getSnapshot().getNodes());
                    }
                } else {
                    inputStarts.addAll(
                        inputCommit.getSnapshot().getStartNodes());
                    ...
                    outputStarts.addAll(
                        outputCommit.getSnapshot().getStartNodes());
                }

                mergeNodeSpaces(inputStarts, outputStarts,
                                inputCommit.getSnapshot(),
                                outputCommit.getSnapshot(), isIntersection,
                                isCompleteMatch, strength);
            }
        } else {
            for (Commit inputCommit : inputCommits) {
                outputBranch.addCommit(inputCommit);
        ...
        if (outputBranchID.equals(RESERVED_ID)) {
            for (String inputBranchID : prunedBranchIDs) {
                fastForwardBranch(targetSpace.getSpaceID(), inputBranchID,
                                  RESERVED_ID);
            }
    ...
    public Map<String, Object> d3GraphDesignSpace(String targetSpaceID) {
        return mapDesignSpaceToD3Format(
            designSpaceRepository.mapDesignSpace(targetSpaceID));
    }

    public void insertDesignSpace(String inputSpaceID1, String inputSpaceID2,
                                  String targetNodeID, String outputSpaceID)
        ...
        validateCombinationalDesignSpaceOperator(inputSpaceID1, inputSpaceID2,
                                                 outputSpaceID);
        ...
        if (outputSpaceID.equals(inputSpaceID1)) {
            ...
            targetNode =
                findNodeCopy(inputSpaceID1, targetNodeID, outputSpaceID);
        }

        deleteNodeCopyIndices(outputSpaceID);

        Set<Edge> removedEdges =
            removeOutgoingEdges(outputSpaceID, targetNode.getNodeID());
        ...
        if (startNode2 != null) {
            ...
            createEdge(outputSpaceID, targetNode.getNodeID(),
                       startNode2.getNodeID());
        }
        for (Node acceptNode2 : acceptNodes2) {
            for (Edge removedEdge : removedEdges) {
                if (removedEdge.hasComponentIDs() &&
                    removedEdge.hasComponentRoles()) {
                    createComponentEdge(outputSpaceID, acceptNode2.getNodeID(),
                                        removedEdge.getHead().getNodeID(),
                                        removedEdge.getComponentIDs(),
                                        removedEdge.getComponentRoles());
                } else {
                    createEdge(outputSpaceID, acceptNode2.getNodeID(),
                               removedEdge.getHead().getNodeID());
                }
            }
        }
        ...
        insertBranch(outputSpaceID, headBranchID1, headBranchID2, targetNodeID,
                     null);

        if (!outputSpaceID.equals(inputSpaceID1) &&
            !outputSpaceID.equals(inputSpaceID2)) {
            ...
            if (acceptNodeIDs1.size() > 0) {
                ...
                if (startNodeIDs2.size() > 0) {
                    for (String acceptNodeID1 : acceptNodeIDs1) {
                        ...
                        for (String startNodeID2 : startNodeIDs2) {
                            createEdge(outputSpaceID, acceptNodeID1,
                                       startNodeID2);
                        }
                    }
                }
    ...
    //    public void matchDesignSpace(String inputSpaceID1, List<String>
    //    inputSpaceIDs2, String outputSpacePrefix) {
    ...
    public Map<String, Object> matchDesignSpaces(List<String> querySpaceIDs,
                                                 List<String> queriedSpaceIDs) {
        List<DesignSpace> querySpaces =
            new ArrayList<DesignSpace>(querySpaceIDs.size());

        for (String querySpaceID : querySpaceIDs) {
            querySpaces.add(loadDesignSpace(querySpaceID, 2));
        }

        List<DesignSpace> queriedSpaces =
            new ArrayList<DesignSpace>(queriedSpaceIDs.size());
        ...
        List<List<DesignSpace>> allMatchSpaces =
            new ArrayList<List<DesignSpace>>(querySpaces.size());

        for (int i = 0; i < querySpaces.size(); i++) {
            allMatchSpaces.add(
                new ArrayList<DesignSpace>(queriedSpaces.size()));
    ...
    public void mergeDesignSpaces(List<String> inputSpaceIDs,
        ...
        mergeDesignSpaces(inputSpaceIDs, inputSpaceIDs.get(0), isIntersection,
                          isCompleteMatch, strength, degree);
    }

    public void mergeDesignSpaces(List<String> inputSpaceIDs,
                                  String outputSpaceID, boolean isIntersection,
                                  boolean isCompleteMatch, int strength,
                                  int degree)
        throws ParameterEmptyException, DesignSpaceNotFoundException,
               DesignSpaceConflictException,
        ...
        mergeDesignSpaces(prunedSpaces, outputSpace, isIntersection,
                          isCompleteMatch, strength, degree);
        ...
        mergeBranches(outputSpaceID, headBranchIDs, isIntersection,
                      isCompleteMatch, strength, degree);
    ...
    private void mergeDesignSpaces(boolean isIntersection,
                                   boolean isCompleteMatch, int strength,
                                   int degree, List<DesignSpace> inputSpaces) {
        if (inputSpaces.size() > 1) {
            mergeDesignSpaces(inputSpaces.subList(1, inputSpaces.size()),
                              inputSpaces.get(0), isIntersection,
                              isCompleteMatch, strength, degree);
        }
    }

    private HashMap<String, Integer> calculateNodeDepths(DesignSpace space) {
    ...
    private void mergeDesignSpaces(List<DesignSpace> inputSpaces,
                                   DesignSpace outputSpace,
                                   boolean isIntersection,
                                   boolean isCompleteMatch, int strength,
                                   int degree) {
        for (DesignSpace inputSpace : inputSpaces) {
            if (outputSpace.hasNodes()) {
                HashMap<String, Node> mergedIDToOutputNode =
                    new HashMap<String, Node>();

                HashMap<String, Set<Node>> idToOutputNodes =
                    new HashMap<String, Set<Node>>();

                HashMap<String, Integer> idToInputDepth =
                    calculateNodeDepths(inputSpace);

                //        		System.out.println(idToInputDepth.toString());

                HashMap<String, Integer> idToOutputDepth =
                    calculateNodeDepths(outputSpace);

                //        		System.out.println(idToOutputDepth.toString());

                Set<Node> priorOutputNodes =
                    new HashSet<Node>(outputSpace.getNodes());

                for (Node inputNode : inputSpace.getNodes()) {
                    for (Node priorOutputNode : priorOutputNodes) {
                        Integer inputDepth =
                            idToInputDepth.get(inputNode.getNodeID());

                        Integer outputDepth =
                            idToOutputDepth.get(priorOutputNode.getNodeID());

                        if (inputNode.hasEdges() &&
                            priorOutputNode.hasEdges() &&
                            (degree == 2 || (degree == 1 &&
                                             inputDepth.intValue() <=
                                                 outputDepth.intValue()) ||
                             (degree == 0 &&
                              inputDepth.intValue() ==
                                  outputDepth.intValue()))) {
                            for (Edge inputEdge : inputNode.getEdges()) {
                                for (Edge priorOutputEdge :
                                     priorOutputNode.getEdges()) {
                                    if (inputEdge.isMatchingTo(priorOutputEdge,
                                                               strength)) {
                                        String mergedID =
                                            inputNode.getNodeID() +
                                            priorOutputNode.getNodeID();

                                        Node outputNode;

                                        if (mergedIDToOutputNode.containsKey(
                                                mergedID)) {
                                            outputNode =
                                                mergedIDToOutputNode.get(
                                                    mergedID);
                                        } else {
                                            if (inputNode.isStartNode() ||
                                                priorOutputNode.isStartNode()) {
                                                outputNode =
                                                    outputSpace
                                                        .createStartNode();
                                            } else if (inputNode
                                                ...
                                                outputNode =
                                                    outputSpace.createNode();
                                            }

                                            mergedIDToOutputNode.put(
                                                mergedID, outputNode);

                                            if (!idToOutputNodes.containsKey(
                                                    inputNode.getNodeID())) {
                                                idToOutputNodes.put(
                                                    inputNode.getNodeID(),
                                                    new HashSet<Node>());
                                        ...
                                        mergedID =
                                            inputEdge.getHead().getNodeID() +
                                            priorOutputEdge.getHead()
                                                .getNodeID();
                                        ...
                                        if (mergedIDToOutputNode.containsKey(
                                            ...
                                            if (outputSuccessor.isStartNode()) {
                                                if (inputEdge.getHead()
                                                        .isAcceptNode() ||
                                                    priorOutputEdge.getHead()
                                                        .isAcceptNode()) {
                                                    outputSuccessor.setNodeType(
                                                        NodeType.ACCEPT
                                                            .getValue());
                                                } else {
                                                    outputSuccessor
                                            ...
                                            idToOutputNodes
                                                .get(inputEdge.getHead()
                                                         .getNodeID())
                                                .add(outputSuccessor);

                                            idToOutputNodes
                                                .get(priorOutputEdge.getHead()
                                                         .getNodeID())
                                                .add(outputSuccessor);
                                        }

                                        Edge outputEdge = outputNode.copyEdge(
                                            priorOutputEdge, outputSuccessor);

                                        if (isIntersection) {
                                            outputEdge.intersectWithEdge(
                                                inputEdge);
                                        } else if (strength > 0) {
                ...
                if (!isIntersection) {
                    mergeSurplusNodes(inputSpace.getNodes(), idToOutputNodes,
                                      outputSpace);

                    mergeSurplusNodes(priorOutputNodes, idToOutputNodes,
                                      outputSpace);
                }
    ...
    private Set<Node> mergeSurplusNode(
        Node surplusNode, HashMap<String, Set<Node>> idToOutputNodes,
    ...
    //    private void mergeDesignSpaces(List<DesignSpace> inputSpaces,
    //    DesignSpace outputSpace, boolean isIntersection,
    ...
    //    			}
    //
    //    			SpaceDiff diff = mergeNodeSpaces(inputStarts,
    //    outputStarts, inputSpace, outputSpace,
    //    					isIntersection, isCompleteMatch,
    //    strength);
    //
    //    			if (!isDiffDeleted &&
    //    inputSpaces.contains(outputSpace)) {
    ...
    //
    //    			mergeNodeSpaces(inputStarts, outputStarts,
    //    inputSpace, outputSpace, isIntersection, isCompleteMatch, strength);
    //    		}
    //    	}
    ////    }

    private Node mergeNodes(Node inputNode, Node outputNode,
                            NodeSpace outputSpace, Stack<Node> inputNodeStack,
                            Stack<Node> outputNodeStack,
                            HashMap<String, Node> mergedIDToOutputNode,
                            HashMap<String, Set<Node>> inputIDToOutputNodes) {
        ...
        if (mergedIDToOutputNode.containsKey(mergerID)) {
            ...
            if (!inputIDToOutputNodes.containsKey(inputNode.getNodeID())) {
                inputIDToOutputNodes.put(inputNode.getNodeID(),
                                         new HashSet<Node>());
            }
    ...
    private SpaceDiff mergeNodeSpaces(List<Node> inputStarts,
                                      List<Node> outputStarts,
                                      NodeSpace inputSpace,
                                      NodeSpace outputSpace,
                                      boolean isIntersection,
                                      boolean isCompleteMatch, int strength) {
        HashMap<String, Set<Node>> inputIDToOutputNodes =
        ...
        for (Node inputStart : inputStarts) {
            ...
            for (Node outputStart : outputStarts) {
                ...
                if (!isIntersection ||
                    isInputStartMatching(inputStart, outputStart, strength)) {
                    mergeNodes(inputStart, outputStart, outputSpace,
                               inputNodeStack, outputNodeStack,
                ...
                while (inputNodeStack.size() > 0 &&
                    ...
                    if (inputNode.hasEdges() && outputNode.hasEdges()) {
                        for (Edge outputEdge : outputNode.getEdges()) {
                            ...
                            for (Edge inputEdge : inputNode.getEdges()) {
                                if (inputEdge.isMatchingTo(outputEdge,
                                                           strength)) {
                                    Node inputSuccessor = inputEdge.getHead();

                                    outputSuccessor = mergeNodes(
                                        inputSuccessor, outputSuccessor,
                                        outputSpace, inputNodeStack,
                                        outputNodeStack, mergedIDToOutputNode,
                                        inputIDToOutputNodes);

                                    if (outputSuccessor !=
                                        outputEdge.getHead()) {
                                        outputEdge = outputEdge.copy(
                                            outputNode, outputSuccessor);
        ...
        for (Node typedInput : inputSpace.getTypedNodes()) {
            if (inputIDToOutputNodes.containsKey(typedInput.getNodeID())) {
                for (Node outputNode :
                     inputIDToOutputNodes.get(typedInput.getNodeID())) {
                    outputNode.setNodeType(typedInput.getNodeType());
                }
            }
        }

        if (inputStarts.size() > 0 &&
            (!isIntersection || outputStarts.size() == 0)) {
            HashMap<String, Node> inputIDToSurplusOutput =
                new HashMap<String, Node>();
            ...
            for (Node inputStart : inputStarts) {
                ...
                if (!inputIDToOutputNodes.containsKey(inputStart.getNodeID())) {
                    inputIDToSurplusOutput.put(
                        inputStart.getNodeID(),
                        outputSpace.copyNode(inputStart));
                }
                ...
                while (inputNodeStack.size() > 0) {
                    ...
                    if (inputNode.hasEdges()) {
                        ...
                        if (inputIDToOutputNodes.containsKey(
                                inputNode.getNodeID())) {
                            outputNodes =
                                inputIDToOutputNodes.get(inputNode.getNodeID());
                        } else {
                            outputNodes = new HashSet<Node>();

                            if (inputIDToSurplusOutput.containsKey(
                                    inputNode.getNodeID())) {
                                outputNodes.add(inputIDToSurplusOutput.get(
                                    inputNode.getNodeID()));
                            }
                        }
                        ...
                        for (Edge inputEdge : inputNode.getEdges()) {
                            ...
                            if (!visitedNodeIDs.contains(
                                    inputSuccessor.getNodeID())) {
                            ...
                            if (inputIDToOutputNodes.containsKey(
                                    inputSuccessor.getNodeID())) {
                                outputSuccessors = inputIDToOutputNodes.get(
                                    inputSuccessor.getNodeID());
                            } else {
                                ...
                                if (inputIDToSurplusOutput.containsKey(
                                        inputSuccessor.getNodeID())) {
                                    outputSuccessors.add(
                                        inputIDToSurplusOutput.get(
                                            inputSuccessor.getNodeID()));
                                } else {
                                    Node outputSuccessor =
                                        outputSpace.copyNode(inputSuccessor);
                            ...
                            if (!inputIDToOutputNodes.containsKey(
                                    inputNode.getNodeID()) ||
                                !inputIDToOutputNodes.containsKey(
                                    inputSuccessor.getNodeID())) {
                                for (Node outputNode : outputNodes) {
                                    for (Node outputSuccessor :
                                         outputSuccessors) {
                                        mergedEdges.add(outputNode.copyEdge(
                                            inputEdge, outputSuccessor));
                                    }
                                }
                            }
                        }
    ...
    public void minimizeDesignSpace(String targetSpaceID)
        ...
        HashMap<String, Set<Edge>> nodeIDsToIncomingEdges =
            targetSpace.mapNodeIDsToIncomingEdges();
        ...
        while (nodeStack.size() > 0) {
            ...
            Set<Edge> minimizableEdges =
                targetSpace.getMinimizableEdges(node, nodeIDsToIncomingEdges);

            if (minimizableEdges.size() == 0 ||
                deletedNodeIDs.contains(node.getNodeID())) {
                nodeStack.pop();
            } else {
                for (Edge minimizableEdge : minimizableEdges) {
                    Node predecessor =
                        nodeIDToNode.get(minimizableEdge.getTail().getNodeID());

                    deletedNodeIDs.add(predecessor.getNodeID());

                    if (nodeIDsToIncomingEdges.containsKey(
                            predecessor.getNodeID())) {
                        for (Edge incomingEdge : nodeIDsToIncomingEdges.get(
                                 predecessor.getNodeID())) {
                            incomingEdge.setHead(node);
                            nodeIDsToIncomingEdges.get(node.getNodeID())
                                .add(incomingEdge);
                        }
                    }
                    ...
                    if (minimizableEdges.size() == 1) {
                        for (Edge edge : predecessor.getEdges()) {
                            if (!edge.equals(minimizableEdge)) {
                                nodeIDsToIncomingEdges
                                    .get(edge.getHead().getNodeID())
                                    .add(node.copyEdge(edge));
                                nodeIDsToIncomingEdges
                                    .get(edge.getHead().getNodeID())
                                    .remove(edge);
                            }
    ...
    public void orDesignSpaces(List<String> inputSpaceIDs, String outputSpaceID)
        ...
        if (startNodeIDs.size() > 1) {
            Node startNodePrime =
                createTypedNode(outputSpaceID, Node.NodeType.START.getValue());

            for (String startNodeID : startNodeIDs) {
                deleteNodeType(outputSpaceID, startNodeID);
                createEdge(outputSpaceID, startNodePrime.getNodeID(),
                           startNodeID);
            }
        }
        ...
        if (acceptNodeIDs.size() > 1) {
            Node acceptNodePrime =
                createTypedNode(outputSpaceID, Node.NodeType.ACCEPT.getValue());

            for (String acceptNodeID : acceptNodeIDs) {
                deleteNodeType(outputSpaceID, acceptNodeID);
                createEdge(outputSpaceID, acceptNodeID,
                           acceptNodePrime.getNodeID());
            }
        }
    ...
    public void partitionDesignSpace(String inputSpaceID,
        ...
        for (Node inputStart : inputSpace.getStartNodes()) {
            ...
            HashMap<String, Node> inputIDToOutputNode =
                new HashMap<String, Node>();

            inputIDToOutputNode.put(inputStart.getNodeID(),
                                    outputSpace.copyNode(inputStart));
            ...
            while (inputNodeStack.size() > 0) {
                ...
                if (inputNode.hasEdges()) {
                    for (Edge inputEdge : inputNode.getEdges()) {
                        ...
                        if (inputIDToOutputNode.containsKey(
                                inputSuccessor.getNodeID())) {
                            outputSuccessor = inputIDToOutputNode.get(
                                inputSuccessor.getNodeID());
                        } else {
                            outputSuccessor =
                                outputSpace.copyNode(inputSuccessor);

                            inputIDToOutputNode.put(inputSuccessor.getNodeID(),
                                                    outputSuccessor);
                        }
    ...
    private void copyDesignSpaceToSnapshot(String inputSpaceID,
                                           String outputBranchID) {
        designSpaceRepository.copyDesignSpaceToSnapshot(inputSpaceID,
                                                        outputBranchID);
    }

    private void copySnapshots(String inputSpaceID, String inputBranchID,
                               String outputSpaceID, String outputBranchID) {
        designSpaceRepository.copySnapshots(inputSpaceID, inputBranchID,
                                            outputSpaceID, outputBranchID);
    }

    public void createEdge(String targetSpaceID, String targetTailID,
                           String targetHeadID) {
        designSpaceRepository.createEdge(targetSpaceID, targetTailID,
                                         targetHeadID);
    }

    private void createEdge(String targetSpaceID, String targetBranchID,
                            String targetTailID, String targetHeadID) {
        designSpaceRepository.createEdge(targetSpaceID, targetBranchID,
                                         targetTailID, targetHeadID);
    }
    ...
    public void createComponentEdge(String targetSpaceID, String targetTailID,
                                    String targetHeadID,
                                    ArrayList<String> componentIDs,
                                    ArrayList<String> componentRoles) {
        designSpaceRepository.createComponentEdge(targetSpaceID, targetTailID,
                                                  targetHeadID, componentIDs,
                                                  componentRoles);
    }

    private void createComponentEdge(String targetSpaceID,
                                     String targetBranchID, String targetTailID,
                                     String targetHeadID,
                                     ArrayList<String> componentIDs,
                                     ArrayList<String> componentRoles) {
        designSpaceRepository.createComponentEdge(targetSpaceID, targetBranchID,
                                                  targetTailID, targetHeadID,
                                                  componentIDs, componentRoles);
    }

    private Node createTypedNode(String targetSpaceID, String nodeType) {
        Set<Node> typedNodes =
            designSpaceRepository.createTypedNode(targetSpaceID, nodeType);
    ...
    private Node createTypedNode(String targetSpaceID, String targetBranchID,
                                 String nodeType) {
        Set<Node> typedNodes = designSpaceRepository.createTypedNode(
            targetSpaceID, targetBranchID, nodeType);
        if (typedNodes.size() > 0) {
            return typedNodes.iterator().next();
        } else {
            return null;
    ...
    private void deleteCommitCopyIndices(String targetSpaceID,
                                         String targetBranchID) {
        designSpaceRepository.deleteCommitCopyIndices(targetSpaceID,
                                                      targetBranchID);
    }

    public void deleteEdge(String targetSpaceID, String targetTailID,
                           String targetHeadID) {
        designSpaceRepository.deleteEdge(targetSpaceID, targetTailID,
                                         targetHeadID);
    }
    ...
    private void deleteNodeType(String targetSpaceID, String targetBranchID,
                                String targetNodeID) {
        designSpaceRepository.deleteNodeType(targetSpaceID, targetBranchID,
                                             targetNodeID);
    }
    ...
    private void fastForwardBranch(String targetSpaceID, String targetBranchID1,
                                   String targetBranchID2) {
        designSpaceRepository.fastForwardBranch(targetSpaceID, targetBranchID1,
                                                targetBranchID2);
    }

    private Branch findBranch(String targetSpaceID, String targetBranchID) {
        Set<Branch> targetBranches =
            designSpaceRepository.findBranch(targetSpaceID, targetBranchID);
    ...
    private Node findNode(String targetSpaceID, String targetNodeID) {
        Set<Node> targetNodes =
            designSpaceRepository.findNode(targetSpaceID, targetNodeID);
    ...
    private Node findNodeCopy(String targetSpaceID1, String targetNodeID,
                              String targetSpaceID2) {
        Set<Node> nodeCopies = designSpaceRepository.findNodeCopy(
            targetSpaceID1, targetNodeID, targetSpaceID2);
    ...
    private Node findNodeCopy(String targetSpaceID, String targetBranchID1,
                              String targetNodeID, String targetBranchID2) {
        Set<Node> nodeCopies = designSpaceRepository.findNodeCopy(
            targetSpaceID, targetBranchID1, targetNodeID, targetBranchID2);
        if (nodeCopies.size() > 0) {
            return nodeCopies.iterator().next();
        } else {
            return null;
    ...
    private String getHeadBranchID(String targetSpaceID) {
        Set<String> headBranchIDs =
            designSpaceRepository.getHeadBranchID(targetSpaceID);
    ...
    private Set<Node> getAcceptNodes(String targetSpaceID,
                                     String targetBranchID) {
        return getNodesByType(targetSpaceID, targetBranchID,
                              Node.NodeType.ACCEPT.getValue());
    }
    ...
    private Set<String> getAcceptNodeIDs(String targetSpaceID,
                                         String targetBranchID) {
        return getNodeIDsByType(targetSpaceID, targetBranchID,
                                Node.NodeType.ACCEPT.getValue());
    }
    ...
    private Set<Node> getNodesByType(String targetSpaceID,
                                     String targetBranchID, String nodeType) {
        return designSpaceRepository.getNodesByType(targetSpaceID,
                                                    targetBranchID, nodeType);
    }

    private Set<String> getNodeIDsByType(String targetSpaceID,
                                         String nodeType) {
    ...
    private Set<String> getNodeIDsByType(String targetSpaceID,
                                         String targetBranchID,
                                         String nodeType) {
        return designSpaceRepository.getNodeIDsByType(targetSpaceID,
                                                      targetBranchID, nodeType);
    }

    private Set<Edge> getOutgoingEdges(String targetSpaceID,
                                       String targetNodeID) {
        return designSpaceRepository.getOutgoingEdges(targetSpaceID,
                                                      targetNodeID);
    }
    ...
    private Set<Edge> getOutgoingEdges(String targetSpaceID,
                                       String targetBranchID,
                                       String targetNodeID) {
        return designSpaceRepository.getOutgoingEdges(
            targetSpaceID, targetBranchID, targetNodeID);
    }

    private Node getStartNode(String targetSpaceID) {
        Set<Node> startNodes =
            getNodesByType(targetSpaceID, Node.NodeType.START.getValue());
        if (startNodes.size() > 0) {
    ...
    private Node getStartNode(String targetSpaceID, String targetBranchID) {
        Set<Node> startNodes = getNodesByType(targetSpaceID, targetBranchID,
                                              Node.NodeType.START.getValue());
        if (startNodes.size() > 0) {
            return startNodes.iterator().next();
        } else {
            return null;
    ...
    private String getStartNodeID(String targetSpaceID) {
        Set<String> startNodeIDs =
            getNodeIDsByType(targetSpaceID, Node.NodeType.START.getValue());
    ...
    private boolean isInputStartMatching(Node inputStart, Node outputStart,
                                         int strength) {
    ...
    private Map<String, Object> mapBranchesToD3Format(
        List<Map<String, Object>> branchMap) {
        ...
        if (branchMap.size() > 0) {
            ...
            Map<Map<String, Object>, Integer> nodeAddresses =
                new HashMap<Map<String, Object>, Integer>();
            Set<String> branchIDs = new HashSet<String>();

            d3Graph.put("spaceID", branchMap.get(0).get("spaceID"));

            Map<String, Object> tail =
                makeD3("knoxID", "head", "knoxClass", "Head");
            Map<String, Object> head =
                makeD3("knoxID", branchMap.get(0).get("headBranchID"),
                       "knoxClass", "Branch");
            ...
            for (Map<String, Object> row : branchMap) {
                ...
                if (tailID != null && headID != null) {
                    tail = makeD3("knoxID", row.get("tailID"), "knoxClass",
                                  "Commit");
                    tail.put("copyIndex", row.get("tailCopyIndex"));
                    head = makeD3("knoxID", row.get("headID"), "knoxClass",
                                  "Commit");
                ...
                if (!branchIDs.contains(branchID) &&
                    row.get("latestCommitID") != null) {
                    branchIDs.add(branchID);
                    tail = makeD3("knoxID", branchID, "knoxClass", "Branch");
                    head = makeD3("knoxID", row.get("latestCommitID"),
                                  "knoxClass", "Commit");
    ...
    private Map<String, Object> mapDesignSpaceToD3Format(
        List<Map<String, Object>> spaceMap) {
        Map<String, Object> d3Graph = new HashMap<String, Object>();
        ...
        for (Map<String, Object> row : spaceMap) {
            ...
            Map<String, Object> tail = makeD3("nodeID", row.get("tailID"),
                                              "nodeType", row.get("tailType"));
            ...
            Map<String, Object> head = makeD3("nodeID", row.get("headID"),
                                              "nodeType", row.get("headType"));
            ...
            Map<String, Object> link =
                makeD3("source", source, "target", target);

            if (row.containsKey("componentRoles") &&
                row.get("componentRoles") != null) {
    ...
    private Map<String, Object> mapDesignSpaceToD3Format(DesignSpace space) {
        ...
        for (Node node : space.getNodes()) {
            nodes.add(makeD3("nodeID", node.getNodeID(), "nodeType",
                             node.getNodeType()));
        ...
        for (Node node : space.getNodes()) {
            if (node.hasEdges()) {
                for (Edge edge : node.getEdges()) {
                    ...
                    Map<String, Object> link = makeD3(
                        "source", nodeIndices.get(edge.getTail().getNodeID()),
                        "target", nodeIndices.get(edge.getHead().getNodeID()));

                    link.put("componentRoles",
                             compRoles.toArray(new String[compRoles.size()]));
    ...
    private Map<String, Object> makeD3(String key1, Object value1, String key2,
                                       Object value2) {
    ...
    private Map<String, Object> makeLink(
        Map<String, Object> tail, Map<String, Object> head,
        List<Map<String, Object>> nodes,
        Map<Map<String, Object>, Integer> nodeAddresses) {
        int source;
    ...
    private void mergeBranch(String targetSpaceID, String inputBranchID,
                             String outputBranchID) {
        designSpaceRepository.mergeBranch(targetSpaceID, inputBranchID,
                                          outputBranchID);
    }

    private void mergeBranch(String inputSpaceID, String inputBranchID,
                             String outputSpaceID, String outputBranchID) {
        designSpaceRepository.mergeBranch(inputSpaceID, inputBranchID,
                                          outputSpaceID, outputBranchID);
    }

    private void mergeVersionHistory(String inputSpaceID,
                                     String outputSpaceID) {
        Set<String> inputBranchIDs = getBranchIDs(inputSpaceID);
        for (String inputBranchID : inputBranchIDs) {
            mergeBranch(inputSpaceID, inputBranchID, outputSpaceID,
                        inputBranchID);
            copySnapshots(inputSpaceID, inputBranchID, outputSpaceID,
                          inputBranchID);
        }
        for (String inputBranchID : inputBranchIDs) {
    ...
    private Set<Edge> removeOutgoingEdges(String targetSpaceID,
                                          String targetNodeID) {
        Set<Edge> removedEdges = getOutgoingEdges(targetSpaceID, targetNodeID);
        if (removedEdges.size() > 0) {
            designSpaceRepository.deleteOutgoingEdges(targetSpaceID,
                                                      targetNodeID);
        }
        return removedEdges;
    }
    ...
    private Set<Edge> removeOutgoingEdges(String targetSpaceID,
                                          ...
                                          String targetNodeID) {
        Set<Edge> removedEdges =
            getOutgoingEdges(targetSpaceID, targetBranchID, targetNodeID);
        if (removedEdges.size() > 0) {
            designSpaceRepository.deleteOutgoingEdges(
                targetSpaceID, targetBranchID, targetNodeID);
        }
        return removedEdges;
    }
    ...
    public void unionBranches(String targetSpaceID,
                              List<String> inputBranchIDs) {
