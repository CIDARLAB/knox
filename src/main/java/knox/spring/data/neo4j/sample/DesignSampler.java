...
public class DesignSampler {
    ...
    public Set<List<String>> sample(int numSamples) {
        ...
        for (int i = 0; i < numSamples; i++) {
            ...
            while (node.hasEdges() &&
                   (!node.isAcceptNode() || rand.nextInt(2) == 1)) {
                ...
                if (edge.hasComponentIDs()) {
                    sample.add(edge.getComponentID(
                        rand.nextInt(edge.getNumComponentIDs())));
                }
    ...
    public Set<List<String>> enumerate(EnumerateType enumerateType,
                                       Integer requestedDesigns) {
        int numberOfDesigns =
            requestedDesigns != null ? requestedDesigns : Integer.MAX_VALUE;
    ...
    private Set<List<String>> dfsEnumerate(int numberOfDesigns) {
        ...
        for (Node start : starts) {
            ...
            Set<List<String>> generatedDesigns =
                dfsEnumerateRecursive(start, designs);

            if (generatedDesigns.size() + currentNumberOfDesigns <
                numberOfDesigns) {
                ...
                Iterator<List<String>> generatedDesignsIterator =
                    generatedDesigns.iterator();
    ...
    private Set<List<String>> dfsEnumerateRecursive(Node node,
                                                    Set<List<String>> designs) {
        ...
        for (Edge edge : node.getEdges()) {
            ...
            allVisitedDesigns.addAll(
                dfsEnumerateRecursive(edge.getHead(), visitedDesigns));
        }
            ...
            while (!edgeStack.isEmpty()) {
                ...
                if (edge.hasComponentIDs()) {
                    ...
                    for (String compID : edge.getComponentIDs()) {
                        if (designs.size() > 0) {
                            for (List<String> design : designs) {
                                List<String> comboDesign =
                                    new LinkedList<>(design);
                ...
                if (!head.hasEdges() || head.isAcceptNode()) {
                    if (designs.size() + currentNumberOfDesigns <
                        numberOfDesigns) {
                        allDesigns.addAll(designs);
                        currentNumberOfDesigns += designs.size();
                    } else {
                        int neededDesigns =
                            numberOfDesigns - currentNumberOfDesigns;
                        Iterator<List<String>> generatedDesignsIterator =
                            designs.iterator();
                        for (int i = 0; i < neededDesigns; i++) {
                            allDesigns.add(generatedDesignsIterator.next());
