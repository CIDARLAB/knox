...
public class SubSpace {
    ...
    private SubSpace(DesignSpace space, List<Node> nodes,
                     HashMap<Part, List<Integer>> partToNodeIndices,
                     int numRootNodes) {
        this.space = space;
    ...
    public SubSpace(DesignSpace space, Device device,
            ...
            for (Integer nodeIndex : partToNodeIndices.get(part)) {
                for (Edge edge : nodes.get(nodeIndex).getEdges()) {
                    if (!outgoingEdges.contains(edge)) {
                        edge.addComponent(part.getID(),
                                          part.getType().getValue());
                    }
                }
            }
        }
    ...
    public void connectByPart(SubSpace subSpace, Part part) {
        for (Integer fromIndex : partToNodeIndices.get(part)) {
            connectToSubSpace(subSpace, part, fromIndex.intValue(),
                              fromIndex.intValue() - nodes.size() +
                                  subSpace.getNumNodes() + 1);
        }
    }
    ...
    public void connectByRuleset(SubSpace subSpace, Ruleset ruleset) {
        ...
        int toIndex =
            ruleset.getIndex() - numRootNodes + subSpace.getNumNodes() + 1;

        connectToSubSpace(subSpace, ruleset.getImplicant(), fromIndex, toIndex);
    }

    public void connectByRuleset(SubSpace subSpace, Ruleset ruleset,
                                 List<Part> adjacentSequence) {
        int fromIndex = ruleset.getIndex() - numRootNodes + nodes.size();

        int toIndex = ruleset.getIndex() - numRootNodes +
                      subSpace.getNumNodes() + adjacentSequence.size();

        connectToSubSpace(subSpace, adjacentSequence, fromIndex, toIndex);
    }

    public void connectToSubSpace(SubSpace subSpace, List<Part> parts,
        ...
        for (int i = 0; i < parts.size(); i++) {
            ...
            System.out.println("connect " + part.getID() + " " + fromIndex +
                               " of " + nodes.size() + " to " + toIndex +
                               " of " + subSpace.getNumNodes());
        }
    }

    public void connectToSubSpace(SubSpace subSpace, Part part, int fromIndex,
                                  int toIndex) {
        ArrayList<String> compIDs = new ArrayList<String>(1);
        ...
        outgoingEdges.add(nodes.get(fromIndex).createEdge(
            subSpace.getNode(toIndex), compIDs, compRoles));

        System.out.println("connect " + part.getID() + " " + fromIndex +
                           " of " + nodes.size() + " to " + toIndex + " of " +
    ...
    public SubSpace copy() { return copyFromIndex(0); }
    ...
    public SubSpace copyByRuleset(Ruleset ruleset, int shift) {
        if (ruleset.hasIndex()) {
            return copyFromIndex(ruleset.getIndex() - numRootNodes +
                                 nodes.size() + shift);
        } else {
            return copy();
        }
    ...
    public SubSpace copyFromIndex(int copyIndex) {
                ...
                for (Edge edge : nodes.get(i).getEdges()) {
                    if (!outgoingEdges.contains(edge)) {
                        if (edge.isComponentEdge()) {
                            ...
                            nodeCopies.get(i - copyIndex)
                                .copyEdge(edge, nodeCopies.get(0));
                        }
                    }
                }
            }
        ...
        HashMap<Part, List<Integer>> partToNodeIndicesCopy =
            new HashMap<Part, List<Integer>>();
        ...
        for (Part part : partToNodeIndices.keySet()) {
            ...
            for (Integer nodeIndex : partToNodeIndices.get(part)) {
                if (nodeIndex.intValue() >= copyIndex) {
                    nodeIndicesCopy.add(
                        new Integer(nodeIndex.intValue() - copyIndex));
                }
            }
        ...
        return new SubSpace(space, nodeCopies, partToNodeIndicesCopy,
                            numRootNodes);
    }
    ...
    public void setNextPart(Part part) {
        if (partToNodeIndices.containsKey(part)) {
            ...
            if (nodeIndices.size() > 0) {
                for (Edge edge :
                     nodes.get(nodeIndices.get(0).intValue()).getEdges()) {
                    if (!outgoingEdges.contains(edge)) {
                        edge.setComponent(part.getID(),
                                          part.getType().getValue());
                    }
                }
            }
    ...
    public DesignSpace getDesignSpace() { return space; }
    ...
    public int getNumRootNodes() { return numRootNodes; }
    ...
    //		int j = 0;
    //
    //		if (nodeIndices1.get(i).intValue() ==
    // nodeIndices2.get(j).intValue()) {
    //			if (i + 1 < nodeIndices1.size()
    //					&& j + 1 < nodeIndices2.size()
    //					&& nodeIndices1.get(i + 1).intValue() ==
    // nodeIndices2.get(j).intValue() + 1
    //					&& nodeIndices2.get(j + 1).intValue() ==
    // nodeIndices1.get(i).intValue() + 1) {
    //				i += 2;
    //				j += 2;
    //			} else {
    //				return false;
    //			}
    //		} else if (nodeIndices1.get(i).intValue() ==
    // nodeIndices2.get(j).intValue() + 1
    //				|| nodeIndices2.get(j).intValue() ==
    // nodeIndices1.get(i).intValue() + 1) {
    //			i++;
    ...
    //				i++;
    //				j++;
    //			} else if (nodeIndices1.get(i).intValue() == nodeIndices2.get(j
    //- 1).intValue() + 1) {
    //				i++;
    //			} else if (nodeIndices2.get(j).intValue() == nodeIndices2.get(i
    //- 1).intValue() + 1) {
    //				j++;
    //			} else {
    //				return false;
