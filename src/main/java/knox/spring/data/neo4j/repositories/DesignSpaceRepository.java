...
//    Collection<Node> findByTitleContaining(@Param("title") String title);
    ...
    @Query("MATCH (d:DesignSpace)-[c1:CONTAINS]->(m:Node)-[e:PRECEDES]->(n:Node)<-[c2:CONTAINS]-(d:DesignSpace) WHERE d.displayID =~ ('(?i).*'+{id}+'.*') RETURN m.displayID as tailID, m.nodeType as tailType, e.componentRole as componentRole, n.displayID as headID, n.nodeType as headType")
